...
# ── helper so we don’t crash on empty crops ──────────────────────────
def _thumb_from_cv2(arr, max_size):
    """
    Return a CTkImage thumbnail or None.
    Skips if arr is None or has zero size.
    """
    if arr is None or getattr(arr, "size", 0) == 0:
        return None
    from PIL import Image                        # local import is fine
...
def refresh_data(map: list):
    ...
    for item in map:
        ...
        button.grid(row=id, column=3, padx=20, pady=10)
         ...
         # ───────── 3 a  SOURCE thumbnail (safe) ─────────
        if "source" in item:
            tk_image = _thumb_from_cv2(                       # ← helper skips None/empty
                item.get("source", {}).get("cv2"),
                (MAPPER_PREVIEW_MAX_WIDTH, MAPPER_PREVIEW_MAX_HEIGHT),
            )
            if tk_image:                                      # only add label if we got one
                source_image = ctk.CTkLabel(
                    scrollable_frame,
                    text=f"S-{id}",
                    width=MAPPER_PREVIEW_MAX_WIDTH,
        ...
        # ───────── 3 b  TARGET thumbnail (safe) ─────────
        if "target" in item:
            tk_image = _thumb_from_cv2(
                item.get("target", {}).get("cv2"),
                (MAPPER_PREVIEW_MAX_WIDTH, MAPPER_PREVIEW_MAX_HEIGHT),
            )
            if tk_image:
                target_image = ctk.CTkLabel(
